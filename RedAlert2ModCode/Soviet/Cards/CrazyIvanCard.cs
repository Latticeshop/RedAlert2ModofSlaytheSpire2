...
public sealed class CrazyIvanCard : CardModel
{
    private static readonly CardValueStore.CardValues Values = SovietCardValues.CrazyIvan;

    public CrazyIvanCard() : base((int)Values.Cost, CardType.Attack, CardRarity.Token, TargetType.AnyEnemy) { }

    public override string PortraitPath => "res://RedAlert2ModResources/images/packed/card_portraits/soviet/ivanicon.png";
        ...
        if (selectedIndex == 0)
        {
            if (play.Target is Creature target)
            {
                ...
                if (existingBomb != null)
                {
                    ...
                    GD.Print($"[CrazyIvanCard] 敌人已有定时炸弹，减少1层倒计时，当前剩余: {existingBomb.Amount}");
                ...
                else
                    {
                        ...
                        GD.Print($"[CrazyIvanCard] 赋予敌人{bombStacks}层定时炸弹");
                    }
            }
        }
    ...
    private async Task ShowDeploySelection(PlayerChoiceContext ctx)
    {
        ...
        if (!unitCards.Any())
        {
            GD.Print("[CrazyIvanCard] 手牌中没有单位卡牌，跳过部署");
            return;
        }
    ...
    private static void EnsureBombPlantAudioPlayer()
    {
        ...
        _bombPlantAudioPlayer.Name = "CrazyIvanBombPlantAudioPlayer";
    ...
    private void PlayBombPlantSound()
    {
        try
        {
            ...
            if (soundFile != null)
            {
                ...
                GD.Print("[CrazyIvanCard] 播放炸弹安装音效");
            }
        }
        catch (Exception ex)
        {
            GD.PrintErr($"[CrazyIvanCard] 播放炸弹安装音效失败: {ex.Message}");
        }
    }
