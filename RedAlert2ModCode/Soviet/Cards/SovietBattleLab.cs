...
public sealed class SovietBattleLab : CardModel
{
	...
	protected override async Task OnPlay(PlayerChoiceContext ctx, CardPlay play)
	{
		...
		// 获得作战实验室能力（使用盟军的BattleLabPower，因为逻辑相同）
		// 这里可能需要创建一个苏军的BattleLabPower，或者共用一个
		// 为了简化，暂时共用
		await PowerCmd.Apply<RedAlert2ModCode.Allies.Powers.BattleLabPower>(new ThrowingPlayerChoiceContext(), Owner.Creature, 1m, Owner.Creature, this);
