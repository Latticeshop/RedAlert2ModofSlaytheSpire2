...
public sealed class HeavyVehicle : IfvVehicleBase
{
	private static readonly CardValueStore.CardValues Values = AlliesCardValues.HeavyVehicle;

	public HeavyVehicle() : base((int)Values.Cost, CardType.Attack, CardRarity.Token, TargetType.AnyEnemy) { }

	public override string PortraitPath => "res://RedAlert2ModResources/images/packed/card_portraits/allies/ifv_type2.png";

	protected override string AttackSoundPath => "res://RedAlert2ModResources/audio/AlliedUnits/IFV/missile.wav";
	...
	protected override async Task ExecuteEffect(PlayerChoiceContext ctx, CardPlay play)
	{
		...
		await PowerCmd.Apply<MegaCrit.Sts2.Core.Models.Powers.VulnerablePower>(
			...
			target,
			DynamicVars["VulnerableStacks"].IntValue,
			Owner.Creature,
			this
		);
