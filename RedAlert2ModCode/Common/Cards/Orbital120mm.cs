...
public sealed class Orbital120mm : CardModel
{
    ...
    public override string PortraitPath => $"res://RedAlert2ModResources/images/packed/card_portraits/Helldivers/Orbital/Orbital120mm_card.png";
    ...
    {
        GD.Print("[Orbital120mm] 卡牌打出开始");

        var power = await Orbital120mmPower.ApplyOrbital120mm(Owner.Creature, IsUpgraded);

        if (power != null)
        {
            GD.Print($"[Orbital120mm] 成功获得轨道120MM能力 - Damage={power.CurrentDamage}x{power.CurrentRepeat}");
            ...
            if (play.Target != null && play.Target.IsAlive)
            {
                ...
                GD.Print($"[Orbital120mm] 赋予目标锁定: {play.Target.Name}");
            }
        }
        else
        {
            GD.PrintErr("[Orbital120mm] 获得轨道120MM能力失败");
            return;
        }

        GD.Print("[Orbital120mm] 卡牌打出完成");
    }
