...
public sealed partial class ProductionQueueSelectionScreen : Control, IOverlayScreen
{
    ...
    public static async Task<List<StopProductionCard.ProductionQueueItem>?> ShowSelectionWithSync(
        List<StopProductionCard.ProductionQueueItem> items, int maxSelection, Player player)
    {
        List<StopProductionCard.ProductionQueueItem> itemsCopy = new(items);

        List<int> selectedIndices = await MultiplayerSyncHelper.ExecuteSyncMultiChoice(player, async () =>
        {
            List<StopProductionCard.ProductionQueueItem>? selected = await ShowSelection(itemsCopy, maxSelection, player);
            if (selected == null) return null;
            
            List<int> indices = new();
            foreach (var item in selected)
            {
                int index = itemsCopy.FindIndex(i => i == item);
                if (index >= 0)
                    indices.Add(index);
            }
            return indices;
        ...
        {
            List<StopProductionCard.ProductionQueueItem> result = new();
            foreach (int index in selectedIndices)
            {
                if (index >= 0 && index < itemsCopy.Count)
